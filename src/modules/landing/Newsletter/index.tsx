...
export function Newsletter() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      toast('Subscribe sucessfull', { type: 'success' })
    }, 1500)
    ...
    <>
      <section className="relative overflow-hidden bg-gradient-to-r from-primary-500 to-primary-700">
        <div className="flex max-w-screen-xl flex-col p-5 md:m-auto md:flex-row md:items-center md:gap-6 md:py-16 xl:py-32">
          ...
          <form className="z-10 mb-4 mt-6 md:m-0 md:flex-[1]" onSubmit={handleSubscribe}>
            <div>
              <label
            ...
            <Button
              type="submit"
              className="my-5 w-full py-6 shadow-lg"
              disabled={subscribeLoading}
            >
              {subscribeLoading ? <LoadingSpin /> : <>Subscribe</>}
            </Button>
          </form>
        </div>
...
function LoadingSpin() {
  return (
    <div role="status">
      <svg
        aria-hidden="true"
        className="mr-2 h-8 w-8 animate-spin fill-primary-700 text-primary-500"
        viewBox="0 0 100 101"
        fill="none"
